...
const countryNameWithFlag = document.getElementById("countryNameWithFlag");
const tagInput = document.getElementById("tagInput");
const tagItemContainer = document.getElementById("tagItemContainer");

tagInput.addEventListener('input', onCountryInputChange);


var countryList = [{ "id": 4, "name": "Afghanistan", "alpha2": "af", "alpha3": "afg" },
...
function renderTags(tagList) {
    tagItemContainer.innerHTML = tagList.map(tag =>
        `
        <span class="tag-item">
        <img class="flag" src = "flags/${tag.alpha2}.png"/>${tag.name}
        <i class="fa fa-times" aria-hidden="true" onclick="deleteTag(${tag.id})"></i>
        </span>
        `).join("");
}
function addTag(countryID) {
    var foundCountry = countryList.find(country => country.id == countryID);
    var index = countryList.indexOf(foundCountry);
    var tagToBeAdded = countryList[index];

    tagList.push(tagToBeAdded);
    ...
    countryNameWithFlag.innerHTML = "";
}

function deleteTag(tagID) {
    var foundTag = tagList.find(tag => tag.id == tagID);
    var index = tagList.indexOf(foundTag);
    var countryToBeAdded = tagList[index];

    tagList.splice(index, 1);
    countryList.unshift(countryToBeAdded);
    renderTags(tagList);
    renderCountryList(countryList);
}
...
function onCountryInputChange() {
    ...
    var filteredCountryList = countryList.filter(country => country.name.toLowerCase().includes(tagInput.value.toLowerCase()));
    dropDown.style.display = "flex"
    renderCountryList(filteredCountryList);
}